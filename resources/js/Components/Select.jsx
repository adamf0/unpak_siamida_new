...
const Select = ({label="",handlerChange=()=>{},items = [],selected=null,loading=false}) => {
    ...
    return <div className="w-full relative">
                ...
                <button onClick={()=>{
                    ...
                    setOpen(!open)
                }} type="button" className={`grid w-full cursor-default grid-cols-1 rounded-md px-4 py-2 border border-gray-300 rounded-md ${!open? "bg-white":"bg-gray-100"} focus:outline-none focus:ring-2 focus:ring-purple-500`} aria-haspopup="listbox" aria-expanded={open}>
                ...
                {open && <ul className="!py-1 !px-1 mb-0 absolute z-10 mt-1 max-h-56 w-full overflow-auto rounded-md bg-white py-1 text-base ring-1 shadow-lg ring-black/5 focus:outline-hidden sm:text-sm" role="listbox">
                    {
                        items.map(item => 
                            <li onClick={()=>{
